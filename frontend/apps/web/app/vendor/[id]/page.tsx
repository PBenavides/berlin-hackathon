        ...
        <div className="col-span-2 space-y-4">
          <Card><CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {v.cases.map((c) => {
                  const p = allProps.find((x) => x.id === c.propertyId);
                  return (
                    <TableRow key={c.id}>
                      <TableCell className="font-mono text-xs">{c.date}</TableCell>
                      <TableCell>{p ? <Link href={`/property/${p.id}/tickets`} className="hover:underline">{p.name}</Link> : c.propertyId}</TableCell>
