We have to implement Hermes as an agent tailored for each of the property owners, each property owner has their own state and context. Therefore we have to create one profile and folder directory for each of these profiles, ALL agent entries are going to be deployed as gateways and with an specific directory. 

The current architecture should be pluggable and compliat with the current backend set of endpoints:
...
After the first draft plan you have to create a clear document on how to test this implementation and what are the steps need it in order to provide the context to this agent. 

Messaging would be fine to test it overall, and we can use Gemini API Keys in order to do that. Preferably. Because its cheap. Also allow images to be sent to this messages endpoint. PLAN and clarify the scope of this task, it should be minimal but clean so think about how to implement this to the current implementation.

Preferred HERMES sources:
